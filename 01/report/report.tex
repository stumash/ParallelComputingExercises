...
The above graph depicts a function rising at an increasing rate. There is not enough data to model
the function exactly, though a low-order polynomial like $O(n^3)$ is expected since the matrix
multplication algorithm used runs in $O(n^3)/k$ where $k$ is the number of cores/threads and is
constant.
